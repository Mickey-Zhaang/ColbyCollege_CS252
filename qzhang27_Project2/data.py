...
class Data:
    ...
    def __str__(self):
        ...
        if self.data is None:
            return "Data object (no data loaded)"
        n, m = self.data.shape[0], self.data.shape[1]
        filepath_str = self.filepath if self.filepath else "None"
        lines = [
            "------------------------------",
            f"{filepath_str} ({n}x{m})",
            "Headers:",
            "\t".join(self.headers) if self.headers else "",
            "------------------------------",
            f"Showing first {min(5, n)}/{n} rows.",
        ]
